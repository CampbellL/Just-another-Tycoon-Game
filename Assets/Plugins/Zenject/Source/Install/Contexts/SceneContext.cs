...
namespace Zenject
{
  public class SceneContext : RunnableContext
  {
    ...
    public void Awake()
    {
      ...
      using (
        ProfileTimers.CreateTimedBlock("Other")
#endif
      {
        Initialize();
      }
...
#if UNITY_EDITOR
      using (
        ProfileBlock.Start("Zenject.SceneContext.Install")
      )
#endif
...
#if UNITY_EDITOR
      using (
        ProfileBlock.Start("Zenject.SceneContext.Resolve")
      )
#endif
    ...
    IEnumerable<DiContainer> GetParentContainers()
    {
      ...
      {
        throw Assert
          .CreateException("SceneContext on object {0} of scene {1} requires at least one of contracts '{2}', but none of the loaded SceneContexts implements that contract.",
          gameObject.name,
